...
def reward(pmu, loads, max_loads, bus_ids, reward_coefficient):
    ...
    for p in range(len(loads[0])):
        ...
        if (pmu[0][int(bus_ids[p]) - 1] < 0.95 or pmu[0][int(bus_ids[p]) - 1] > 1.05):
...
def pmu_ok(pmu, bus_ids):
    print(bus_ids)
    for id in bus_ids:
       if (pmu[0][int(id) - 1] < 0.95 or pmu[0][int(id) - 1] > 1.05):
            return False
    return True
...
def begin_uniform_loadshed(case_file):
    ...
    for id in range(len(Bus_ids)):
        if (rarray[0][int(Bus_ids[id]) - 1] < 0.95 or rarray[0][int(Bus_ids[id]) - 1] > 1.05):
...
def begin_policy_rollout(case_name, depth, reward_list):
    ...
    while True:
        ...
        #print(len(rarray))
        for p in Bus_ids:
            if (rarray[0][int(p) - 1] < 0.95 or rarray[0][int(p) - 1] > 1.05):
                print('Solved it!')
                dont_break = True
...
def generate_reward_list(length, case_file):
    Initialize_Case(case_file)
    rarray, Ok_Solution, localMin, localMax = Solve_Steady()
    violationBuses = []
    for y in range(0, len(rarray[0])):
        if rarray[0][y] < 0.95:
            print rarray[0][y]
            violationBuses.append(y)
            
    reward_list = [1.0 for x in range(length)]
    high_priority = random.sample(range(len(violationBuses)), int(len(violationBuses)/3))
   
    for h in high_priority:
        reward_list[violationBuses[h]] = 3.0
    
    print high_priority
    med_choices = []
    for m in range(len(reward_list)):
        if (reward_list[m] != 2.0):
            if m in violationBuses and reward_list[m] != 3.0:
                med_choices.append(m)
    
    print med_choices
    print rarray[0]
    medium_priority = random.sample(med_choices, int(len(med_choices)/3))
